from mainapp.models import Elevator, ElevatorRequest, ElevatorSystem
...
def move_elevator(elevator_object: Elevator, elevator_system: ElevatorSystem):
    ...
    for elev_request in requests_pending:
        ...
        # Invalid Cases, Sometime data can be random though api call because there is not such fixed value
        # at user end (as real elevator) so we have to take care of this conditions
        # 1
        if request_destination < 0 or request_destination > elevator_system.number_of_floor or request_start < 0 or request_start > elevator_system.number_of_floor:
            elev_request.is_active = False
            elev_request.save()
            continue
        # 2
        # start floor and destination floor can't be same 
